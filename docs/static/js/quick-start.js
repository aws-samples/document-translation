...
function updateFormFields() {
	...
	if (
		(form.cognitoLocalUsers.checked &&
			form.cognitoLocalUsersMfa.value == "required") ||
		(form.cognitoLocalUsers.checked &&
			form.cognitoLocalUsersMfa.value == "optional")
	) {
...
function handleForm(event) {
	...
	if (data.piiDetectionEnable === "on") {
		...
		appendStep(
			"Set PII lifecycle",
			`export translationLifecyclePii="${data.piiDetectionLifecycle}"`
		);
	}
	...
	if (data.customDomainEnable === "on") {
		...
		appendStep(
			...
			`export webUiCustomDomain="${data.customDomainName}"`
		);
		appendStep(
			"Set custom domain certificate",
			`export webUiCustomDomainCertificate="${data.customDomainCert}"`
		);
	}
	if (data.sourceGitService) {
		switch (data.sourceGitService) {
			case "codecommit":
				...
				appendStep(
					"Set repo name",
					`export sourceGitRepo="${data.sourceGitRepo}"`
				);
				appendStep(
					"Create CodeCommit repo",
					`aws codecommit create-repository --repository-name ${data.sourceGitRepo}`
				);
				break;
			case "github":
				...
				appendStep(
					"Set source service",
					`export sourceGitService="${data.sourceGitService}"`
				);
				appendStep(
					"Set repo name",
	...
	if (data.cognitoLocalUsers === "on") {
		...
		appendStep(
			"Set MFA enforcement",
			`export cognitoLocalUsersMfa="${data.cognitoLocalUsersMfa}"`
		);
	...
	if (data.cognitoSamlUsers === "on") {
		...
		appendStep(
			"Set SAML metadata URL",
			`export cognitoSamlMetadataUrl="${data.cognitoSamlMetadataUrl}"`
		);
	}
	...
	if (data.translation === "on") {
		...
		appendStep(
			...
			`export translationLifecycleDefault="${data.translationLifecycleDefault}"`
		);
	}
	if (data.readable === "on") {
		...
		appendStep(
			"Set Readable Bedrock region",
			`export readableBedrockRegion="${data.readableBedrockRegion}"`
		);
	}
	...
	appendStep(
		...
		`cdk bootstrap aws://${data.accountId}/\${AWS_REGION}`
	);
