...
export class dt_parseS3Key extends Construct {
	...
	constructor(scope: Construct, id: string, props: props) {
		...
		const general_setScopeToStage = new sfn.Pass(this, "general_setScopeToStage", {
			parameters: {
		...
		// CHOICE | STAGE
		const general_choiceIsStageType = new sfn.Choice(this, "general_choiceIsStageType")
		const stageIsUploadCondition = sfn.Condition.stringEquals("$.result.stage.value", "upload");
		const stageIsOutputCondition = sfn.Condition.stringEquals("$.result.stage.value", "output");
		...
		const output_choiceIsAccessCheck = new sfn.Choice(this, "output_choiceIsAccessCheck")
		const output_fileIsAccessCheckCondition = sfn.Condition.stringEquals("$.segmentKey[4]", ".write_access_check_file.temp");
		const output_fileIsAccessCheckEnd = new sfn.Succeed(this, "output_fileIsAccessCheckEnd", {
		...
		const output_choiceIsDetails = new sfn.Choice(this, "output_choiceIsDetails")
		const output_fileIsDetailsCondition = sfn.Condition.stringEquals("$.segmentKey[5]", "details");
		const output_details_setFileName = new sfn.Pass(this, "output_details_setFileName", {
			resultPath: "$.result.fileName",
			parameters: {
		...
		const output_translation_setFileName = new sfn.Pass(this, "output_translation_setFileName", {
			resultPath: "$.result.fileName",
			parameters: {
				value: sfn.JsonPath.stringAt("$.segmentKey[5]"),
			},
		})
		const output_translation_segmentFileName = new sfn.Pass(this, "output_translation_segmentFileName", {
			parameters: {
		...
		const output_translation_setLanguage = new sfn.Pass(this, "output_translation_setLanguage", {
		...
		this.sfnMain = new dt_stepfunction(
			...
			`${cdk.Stack.of(this).stackName}_TranslationParseS3Key`,
			{
				...
				definition: general_segmentKey
					.next(general_setScopeToStage)
					.next(general_choiceIsStageType
						.when(
							stageIsUploadCondition,
							upload_setFileName
						)
						.when(
							...
							output_choiceIsAccessCheck
								...
								.otherwise(upload_setJobFolder
									.next(output_choiceIsDetails
										.when(
											output_fileIsDetailsCondition,
											output_details_setFileName)
										.otherwise(output_translation_setFileName
