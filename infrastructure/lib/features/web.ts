	...
	constructor(scope: Construct, id: string, props: props) {
		...
		if (props.webUiCustomDomain && props.webUiCustomDomainCertificate) {
			const certificate = acm.Certificate.fromCertificateArn(
				this,
				"Certificate",
				props.webUiCustomDomainCertificate,
			);
		...
		if (props.development) {
			const reactDevelopmentUrl = "http://localhost:3000/";
			cfnUserPoolClient.callbackUrLs.push(reactDevelopmentUrl);
			cfnUserPoolClient.logoutUrLs.push(
				reactDevelopmentUrl + props.signOutSuffix,
			);
		}
